...
<body style="padding: 16px 24px;">
        ...
        function getSupplierData(firm) {
            ...
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    loadDataIntoDdlGeneral(result, supplierDdl, "Select Supplier", "supplier", "");
    ...
    <script>
        ...
        function loadViewData() {
            ...
            xmlhttp.open("POST", `services/getDetailedReport.php`, true);
            xmlhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            xmlhttp.send(`frommonth=${fromMonthDdl.value}&fromyear=${fromYearDdl.value}&tomonth=${toMonthDdl.value}&toyear=${toYearDdl.value}&firm=${firmDdl.value}&supplier=${supplierDdl.value}&view=true`);
        ...
        function loadDataIntoTable(resultArray) {
            ...
            for (let i = 0; i < resultArray[0].length; i++) {
                finalResultString += `<th>${resultArray[0][i]}</th>`;
            }
            finalResultString += `</tr>`;
            ...
            for (let i = 1; i < lastRow; i++) {
                finalResultString += `<tr>`;
                for (const key in resultArray[i]) {
                    const value = resultArray[i][key];
                    finalResultString += `<td>${formatNumberToIndian(value)}</td>`;
                }
                finalResultString += `</tr>`;
            }
            ...
            // adding the footer
            finalResultString += `<tfoot>`;
            finalResultString += `<tr>`;
            for (let i = 0; i < resultArray[lastRow].length; i++) {
                finalResultString += `<th>${formatNumberToIndian(resultArray[lastRow][i])}</th>`;
            }
            finalResultString += `</tr>`;
            finalResultString += `</tfoot>`;
    ...
    <script>
        ...
        submitBtn.addEventListener("click", e => {
            ...
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    var result = JSON.parse(this.responseText);
                    filename = getMonthWord(fromMonthDdl.value) + "_" + fromYearDdl.value + "_" + getMonthWord(toMonthDdl.value) + "_" + toYearDdl.value + "_" + result.id + "_detailed_report";
            ...
            xmlhttp.send(`frommonth=${fromMonthDdl.value}&fromyear=${fromYearDdl.value}&tomonth=${toMonthDdl.value}&toyear=${toYearDdl.value}&firm=${firmDdl.value}&supplier=${supplierDdl.value}&view=`);
        });
    </script>
    <!-- load products data -->
    <script>
