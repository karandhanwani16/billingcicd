...
$query = "";
$filename = getMonthWord($frommonth) . "_" . $fromyear . "_" . getMonthWord($tomonth) . "_" . $toyear . "_" . $user_id . "_credit_note_report";

if ($firm ==  "") {
    $query = "select MONTH(credit_note_date),YEAR(credit_note_date),credit_note_id,firm_id,credit_note_date from credit_note where " . getPeriodQuery($frommonth, $fromyear, $tomonth, $toyear);
} else {
    $query = "select MONTH(credit_note_date),YEAR(credit_note_date),credit_note_id,firm_id,credit_note_date from credit_note where firm_id = " . $firm . " and (" . getPeriodQuery($frommonth, $fromyear, $tomonth, $toyear) . ")";
}
...
// iterating through the credit_note Id to get the credit_note products details in
// MONTH,YEAR,credit_note_id,Invoice_products_value,Invoice_products_percentage,firm_id,sgst,cgst,igst
while ($row = $result->fetch_assoc()) {
    $credit_noteProductsQuery = "select MONTH(i.credit_note_date) as month,YEAR(i.credit_note_date) as year,ip.credit_note_id,ip.credit_note_products_value,ip.credit_note_products_percentage,i.firm_id,i.credit_note_sgst_percentage,i.credit_note_cgst_percentage,i.credit_note_igst_percentage from credit_note_products ip,credit_note i where i.credit_note_id = ip.credit_note_id and i.credit_note_id = " . $row["credit_note_id"];
    $credit_noteProductsResults = $con->query($credit_noteProductsQuery);
    while ($credit_noteProductsRow = $credit_noteProductsResults->fetch_assoc()) {
        $detailsInArray[] = $credit_noteProductsRow;
    }
}
...
foreach ($yeargrouped as $year => $yearArray) {
    ...
    foreach ($monthGrouped as $month => $monthArray) {
        ...
        foreach ($firmGroupedArray as $key => $value) {
            // key here is firm id
            // getting the single array of firm with the credit_note products
            $currentArray = $firmGroupedArray[$key];
            // grouping the current firm array according to the credit_note id
            $credit_noteGroupedArray = group_by2($currentArray, "credit_note_id");
            // iterating through each of credit_note id firm and getting rounded off total
            foreach ($credit_noteGroupedArray as $credit_noteArrayKey => $credit_noteArrayValue) {
                $singleCrediteNoteProducts = $credit_noteGroupedArray[$credit_noteArrayKey];
                $tempCrediteNoteTotal = 0;
                $tempGstTotal = 0;
                for ($i = 0; $i < count($singleCrediteNoteProducts); $i++) {
                    $rowPercentage = floatval($singleCrediteNoteProducts[$i]["credit_note_products_percentage"]);
                    $rowTotalValue = floatval($singleCrediteNoteProducts[$i]["credit_note_products_value"]);
                    if ($rowPercentage == 0) {
                        $credit_noteProductRowValue = $rowTotalValue;
                    } else {
                        $credit_noteProductRowValue = $rowTotalValue * ($rowPercentage / 100);
                    }
                    $tempCrediteNoteTotal  = $tempCrediteNoteTotal +  $credit_noteProductRowValue;
                }
                $tempGstTotal = ($tempCrediteNoteTotal * (floatval($singleCrediteNoteProducts[0]["credit_note_sgst_percentage"]) / 100)) + ($tempCrediteNoteTotal * (floatval($singleCrediteNoteProducts[0]["credit_note_cgst_percentage"]) / 100)) + ($tempCrediteNoteTotal * (floatval($singleCrediteNoteProducts[0]["credit_note_igst_percentage"]) / 100));
                $totalArray[$key]["gross_total"] = $totalArray[$key]["gross_total"] + $tempCrediteNoteTotal;
...
function getPeriodQuery($frommonth, $fromyear, $tomonth, $toyear)
{
    ...
    for ($i = 0; $i < count($arr); $i++) {
        if ($i == (count($arr) - 1)) {
            $finalQuery .= "(MONTH(credit_note_date) = " . $arr[$i][0] . " and YEAR(credit_note_date) = " . $arr[$i][1] . ")";
        } else {
            $finalQuery .= "(MONTH(credit_note_date) = " . $arr[$i][0] . " and YEAR(credit_note_date) = " . $arr[$i][1] . ") or ";
        }
    }
    return $finalQuery;
}
