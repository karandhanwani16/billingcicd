...
$invoiceCopy = isset($_GET["id"]);
$curentId = 0;

if ($invoiceCopy) {
    $curentId = $_GET["id"];
    $invoiceDetails = getTableDetails("invoice", "invoice_id", $curentId, $con);
}
...
<body>
    ...
    <form action="#">
        <div class="inp-row adj-row row-5">
            ...
            <div class="inp-group">
                ...
                <!-- <input type="date" value="<?php //echo  $invoiceCopy ? $invoiceDetails["invoice_date"] : $currentDate; 
                                                ?>" class="inp required" id="txtinvoicedate" data-id="txtinvoicedate" /> -->
                <input type="date" value="<?php echo $currentDate; ?>" class="inp required" id="txtinvoicedate" data-id="txtinvoicedate" />
                <div class="error-text" data-id="txtinvoicedate">Cannot leave this field blank</div>
            </div>
            <!-- inp group end -->
            <div class="inp-group">
                <div class="inp-label">Invoice PO No.</div>
                <input type="text" value="<?php echo  $invoiceCopy ? $invoiceDetails["invoice_other_ref"] : ""; ?>" class="inp required" id="txtinvoiceotherreference" placeholder="Invoice PO No." data-id="txtinvoiceotherreference" />
        ...
        <div class="inp-row adj-row row-5">
            <div class="inp-group">
                ...
                <input type="text" value="" class="inp required <?php echo $user_type != "super_admin" ? "disabled" : ""; ?>" tabindex="-1" id="txtinvoiceno" placeholder="Invoice No." data-id="txtinvoiceno" />
                <div class="error-text" data-id="txtinvoiceno">Cannot leave this field blank</div>
            </div>
            <!-- inp group end -->
            <div class="inp-group">
                <div class="inp-label">GST Percentage</div>
                <input type="text" value="<?php echo  $invoiceCopy ? (strtolower($invoiceDetails["invoice_place_of_supply"]) == "maharastra" ? $invoiceDetails["invoice_sgst_percentage"] * 2 : $invoiceDetails["invoice_igst_percentage"]) : ""; ?>" class="inp required" id="txtgst" placeholder="GST Percentage" data-id="txtgst" />
        ...
        <div class="inp-row adj-row row-5" style="justify-content: flex-start;">
            <div class="inp-group">
                ...
                <input type="text" value="<?php echo  $invoiceCopy ? $invoiceDetails["invoice_place_of_supply"] : "Maharashtra"; ?>" class="inp required" id="txtplaceofsupply" placeholder="Invoice No." data-id="txtplaceofsupply" />
            ...
            <div class="inp-group" style="margin-left: 28px;">
                ...
                <input type="text" value="<?php echo  $invoiceCopy ? $invoiceDetails["invoice_ref"] : ""; ?>" class="inp" id="txtinvoicereference" placeholder="Invoice Reference" data-id="txtinvoicereference" />
                <div class="error-text" data-id="txtinvoicereference">Cannot leave this field blank</div>
            </div>
            <!-- inp group end -->
            <!-- <div class="inp-group" style="margin-left: 28px;">
                <div class="inp-label">PAN No.</div>
                <input type="text" value="<?php //echo  $invoiceCopy ? $invoiceDetails["invoice_pan"] : ""; 
                                            ?>" class="inp" id="txtinvoicepan" placeholder="Invoice PAN No." data-id="txtinvoicepan" />
    ...
    <script>
        ...
        if ($invoiceCopy) {
            $query = "select * from invoice_products where invoice_id = " . $curentId;
            $result = $con->query($query);
            $countOfRows = $result->num_rows;
        ...
        submitBtn.addEventListener("click", e => {
            if (banktable.invoiceRows.length != 0) {
                ...
                if (isValid()) {
                    ...
                    xmlhttp.open("POST", `services/uploadInvoice.php`, true);
    ...
    <script>
        let selectedFirm = "<?php echo $invoiceCopy ? $invoiceDetails["firm_id"] : ""; ?>";
        let selectedBank = "<?php echo $invoiceCopy ? $invoiceDetails["firm_bank_id"] : ""; ?>";
        let selectedSupplier = "<?php echo $invoiceCopy ? $invoiceDetails["supplier_id"] : ""; ?>";
        ...
        if (selectedFirm !== "") {
            ...
            getInvoiceNo(selectedFirm, invoiceDate.value);
        }
