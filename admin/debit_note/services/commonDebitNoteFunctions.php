...
function generateDebitNotePdfHtml($debit_note_id, $con)
{

    $debitNoteDetails = getTableDetails("debit_note", "debit_note_id", $debit_note_id, $con);
    $supplierDetails = getTableDetails("supplier", "supplier_id", $debitNoteDetails["supplier_id"], $con);

    $financialYear = getFinancialyear($debitNoteDetails["debit_note_date"]);
    $debit_noteProductsDetails =  getTableDetails("debit_note_products", "debit_note_id", $debit_note_id, $con);

    $firmDetails = getTableDetails("firm", "firm_id", $debitNoteDetails["firm_id"], $con);
    $bankDetails = getTableDetails("firm_bank", "firm_bank_id", $debitNoteDetails["firm_bank_id"], $con);
    ...
    $finalString .= " <table style='padding-top: 16px;padding-bottom: 8px;width:100%;'> <tr> <td style='width: 33%;text-align:center;'></td><td style='width: 33%;text-align:center;'> <h3>DEBIT NOTE</h3> </td><td style='width: 33%;text-align:center;'> <p>(DUPLICATE FOR BUYER)</p></td></tr></table>";
    ...
    // debit_note no & etc
    $finalString .= "<td style='border-bottom:1px solid #000;border-right:1px solid #000;'> <div class='header-label'>DebitNote No.</div><div class='header-data'>" . $financialYear . "/" . $debitNoteDetails["debit_note_no"] . "</div></td><td style='border-bottom:1px solid #000;'> <div class='header-label'>Dated</div><div class='header-data'>" . date("j-M-y", strtotime($debitNoteDetails["debit_note_date"])) . "</div></td></tr><tr> <td colspan='2' style='border-bottom:1px solid #000;'> <div class='header-label'>PO No.</div><div class='header-data'>" . $debitNoteDetails["debit_note_other_ref"] . "</div></td></tr>";
    // $finalString .= "<td style='border-bottom:1px solid #000;border-right:1px solid #000;'> <div class='header-label'>DebitNote No.</div><div class='header-data'>" . $debitNoteDetails["debit_note_no"] . "/" . $financialYear . "</div></td><td style='border-bottom:1px solid #000;'> <div class='header-label'>Dated</div><div class='header-data'>" . date("j-M-y", strtotime($debitNoteDetails["debit_note_date"])) . "</div></td></tr><tr> <td style='border-bottom:1px solid #000;'> <div class='header-label'>PO No.</div><div class='header-data'>" . $debitNoteDetails["debit_note_other_ref"] . "</div></td><td style='border-bottom:1px solid #000;border-left:1px solid #000;'> <div class='header-label'>PAN No.</div><div class='header-data'>" . $debitNoteDetails["debit_note_pan"] . "</div></td></tr>";
    //supplier details
    $finalString .= "<tr> <td style='width: 50%;border-right:1px solid #000;'> <p>Buyer (Bill to)</p><p> <b>" . $supplierDetails["supplier_name"] . "</b> </p><p>" . $supplierDetails["supplier_address"] . "</p><p> GSTIN/UIN : " . $supplierDetails["supplier_gst_no"] . " </p><p> PAN/IT NO: " . getPanFromGST($supplierDetails["supplier_gst_no"]) . " </p><p> State Name: " . $supplierDetails["supplier_state"] . " </p><p> Code: " . $supplierDetails["supplier_state_code"] . " </p><p> Place of Supply: " . $debitNoteDetails["debit_note_place_of_supply"] . " </p></td><td colspan='2' style='border-bottom: 1px solid #000;border-right: 1px solid #000;'><p style='margin-bottom:12px;'><b>Reference</b></p><p>" . getMultiLineText($debitNoteDetails["debit_note_ref"]) . "</p></td></tr></table>";
    ...
    // print_r($debit_noteProductsDetails);

    $subTotal = 0;
    $rowTotal = is_null($debit_noteProductsDetails) ? 0 : count($debit_noteProductsDetails);
    for ($i = 0; $i < $rowTotal; $i++) {
        ...
        // main charges
        $finalString .= "<p><b>" . $debit_noteProductsDetails[$i]["debit_note_products_name"] . "</b></p>";
        // // prefix
        $finalString .= "</td>";
        // HSN
        $finalString .= "<td style='border-right: 1px solid #000;vertical-align: top;text-align:center;'>" . $debit_noteProductsDetails[$i]["debit_note_products_hsn"] . "</td>";
        // row total
        $rowAmountTotal = floatval($debit_noteProductsDetails[$i]["debit_note_products_value"]);
    ...
    // calculations
    $cGstPercentage = $debitNoteDetails["debit_note_cgst_percentage"];
    $sGstPercentage = $debitNoteDetails["debit_note_sgst_percentage"];
    $iGstPercentage = $debitNoteDetails["debit_note_igst_percentage"];
    ...
    $finalString .= "</table><p style='width: 100%;text-align: center;margin-top: 12px;'>This is Computer Generated DebitNote</p></body></html>";
...
function getTableDetails($tableName, $primaryIdColumnName, $primaryId, $con)
{
    ...
    if ($countOfRows != 0) {
        if ($tableName == "debit_note_products") {
            $rowArray = array();
...
function formatDateStringDebitNote($date_string)
{
