...
<body>
    ...
    <div class="hidden popup">
        ...
        <div class="debit_note-details">
            <div class="roww">
                <div class="data-label">Debit Note ID</div>
                <div class="data-value" id="lbldebit_noteid">1</div>
            </div>
            <div class="roww">
            ...
            <div class="roww">
                <div class="data-label">Debit Note No.</div>
                <div class="data-value" id="lbldebit_noteno">1250</div>
            </div>
            <div class="roww">
                <div class="data-label">Debit Note Date.</div>
                <div class="data-value" id="lbldebit_notedate">2022-10-20</div>
            </div>
        </div>
    ...
    <script>
        ...
        let debitNoteIdLabel = document.querySelector("#lbldebit_noteid");
        let firmLabel = document.querySelector("#lblfirm");
        let supplierLabel = document.querySelector("#lblsupplier");
        let debitNoteNoLabel = document.querySelector("#lbldebit_noteno");
        let debitNoteDateLabel = document.querySelector("#lbldebit_notedate");
        ...
        function addPopupListener() {
            ...
            editReuestButtons.forEach(editReuestButton => {
                editReuestButton.addEventListener("click", e => {
                    let currentDebitNoteId = editReuestButton.attributes["data-id"].value;
                    openPopup();
                    getInvoiceData(currentDebitNoteId);
                });
            });
        }
        ...
        // main functions
        // load data function
        function getInvoiceData(debit_note_id) {
            var xmlhttp = new XMLHttpRequest();
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    var result = JSON.parse(this.responseText);
                    if (!(Object.keys(result).length === 0)) {
                        loadDataIntoPopUpLabels(result.debit_note_id, result.firm_name, result.supplier_name, result.debit_note_no, result.debit_note_date);
                        submitButton.classList.remove("disabled");
                        submitButton.attributes["data-id"].value = debit_note_id;
                    } else {
            ...
            xmlhttp.open("POST", `services/getDebitNoteLabelData.php`, true);
            xmlhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            xmlhttp.send(`id=${debit_note_id}`);
        }
        // load labels
        function loadDataIntoPopUpLabels(debit_note_id, firm_name, supplier_name, debit_note_no, debit_note_date) {
            debitNoteIdLabel.innerHTML = debit_note_id;
            firmLabel.innerHTML = firm_name;
            supplierLabel.innerHTML = supplier_name;
            debitNoteNoLabel.innerHTML = debit_note_no;
            debitNoteDateLabel.innerHTML = debit_note_date;
        }
    </script>
    <!-- pop up end -->
    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function() {
            ...
            var dataTable = $('#sample_data').DataTable({
                ...
                "order": [],
                "ajax": {
                    url: "services/getDebitNoteData.php",
                    type: "POST"
                },
            ...
            function downloadPdf(btn, id) {
                ...
                xmlhttp.open("POST", `services/getDebitNotePdf.php`, true);
                xmlhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                xmlhttp.send("id=" + debit_noteId);
            }
    ...
    <script>
        submitButton.addEventListener("click", e => {
            let currentDebitNoteId = submitButton.attributes["data-id"].value;
            ...
            if (editMessage.value != "") {
                ...
                xmlhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                xmlhttp.send(`id=${currentDebitNoteId}&message=${editMessage.value}`);
            } else {
