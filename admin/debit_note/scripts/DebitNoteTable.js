...
class DebitNoteTable {
    ...
    insertRow(mainText, hsn, amount) {
        let tempObject = new DebitNoteRow(this.currentId, mainText, hsn, amount);
        this.debitnoteRows.push(tempObject);
        this.incrementId();
    }
    removeRow(id) {
        this.debitnoteRows = this.debitnoteRows.filter(function (item) {
            return item.id != id;
        });
    }
    displayRows() {
        let finalResult = "";
        let count = 1;
        this.debitnoteRows.forEach((row) => {
